...
/*======Eureka
gcc -lm -lgsl -lgslcblas -O3 -I/opt/local/include -L/opt/local/lib -c -o orbit.o orbit.c
...
g++ -O3 -ffast-math -Isrc/ -c -o foo.o foo.cc  -Lobj -lPot -lOther -lm
g++ -O3 -ffast-math -Isrc/  -lm -I/usr/local/include/gsl/include -L/usr/local/include/gsl/lib -o orbit.out orbit.o foo.o -lgsl -lgslcblas -Lobj -lPot -lOther
========Windows
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=27)
    {
      printf("Correct usage: ./a.out x y z vx vy vz M_LMC rs_LMC x_LMC y_LMC z_LMC vx_LMC vy_LMC vz_LMC Mprog tmax M_NFW rs_NFW c_NFW x_sat y_sat z_sat vx_sat vy_sat vz_sat pid\n");
      exit(1);
    }
  ...
  FILE *fileout;
  char fname[999];
  // sprintf(fname,"secondorbits/orbit_%d.txt",pid);
  ...
  for(i=0;i<3;i++)
    {
      ...
      pdata[2].x[i] = 0.;
      pdata[2].v[i] = 0.;
      pdata[3].x[i] =  x_sat[i]+100.;
      pdata[3].v[i] = -v_sat[i]+100.;
    }
  ...
  //sprintf(fname,"final_coords_%d.txt",pid);
  // sprintf(fname,"orbits_fwd/orbit_fwds_%d.txt",pid);
  // fileout = fopen(fname,"w");


  for(axes=0;axes<3;axes++)
    {
      pdata[0].x[axes] = x_sat[axes];
      // pdata[0].v[axes] *= -1;
      pdata[0].v[axes] = v_sat[axes];
    }
