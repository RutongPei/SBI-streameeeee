...
/*======Eureka
gcc -lm -lgsl -lgslcblas -O3 -I/opt/local/include -L/opt/local/lib -c -o one_plummer_with_stream.o one_plummer_with_stream.c
g++ -O3 -ffast-math -Isrc/ -c -o foo.o foo.cc  -Lobj -lPot -lOther -lm
g++ -O3 -ffast-math -Isrc/  -lm -lgsl -lgslcblas -I/opt/local/include -L/opt/local/lib -o a.out one_plummer_with_stream.o foo.o -Lobj -lPot -lOther

icc -lgsl -lgslcblas -I/opt/local/include -L/opt/local/lib -Ofast -c -o one_plummer_with_stream.o one_plummer_with_stream.c
icc -Ofast -Isrc/ -c -o foo.o foo.cc -Lobj -lPot -lOther -lm 
icc -Ofast -Isrc/ -lgsl -lgslcblas -I/opt/local/include -L/opt/local/lib -o a.out one_plummer_with_stream.o foo.o -lm -Lobj -lPot -lOther 
...
g++ -O3 -ffast-math -Isrc/ -c -o foo.o foo.cc  -Lobj -lPot -lOther -lm
g++ -O3 -ffast-math -Isrc/  -lm -I/usr/local/include/gsl/include -L/usr/local/include/gsl/lib -o a.out one_plummer_with_stream.o foo.o -lgsl -lgslcblas -Lobj -lPot -lOther
========Windows
gcc -lm -lgsl -lgslcblas -O3 -I"C:/cygwin64/usr/include" -L"C:/cygwin64/lib" -c -o one_plummer_with_stream.o one_plummer_with_stream.c
g++ -O3 -ffast-math -Isrc/ -c -o foo.o foo.cc  -Lobj -lPot -lOther -lm
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=29)
    {
      printf("Correct usage: ./a.out x y z vx vy vz M_LMC rs_LMC x_LMC y_LMC z_LMC vx_LMC vy_LMC vz_LMC Mprog tmax M_NFW rs_NFW c_NFW x_sat y_sat z_sat vx_sat vy_sat vz_sat pid M_sat rs_sat\n");
      exit(1);
    }
  ...
  FILE *fileout;
  char fname[999];
  // sprintf(fname,"orbits_final/orbit_%d.txt",pid);
  ...
  printf("Impact pid: %d with M_sat=%f, rs_sat=%f,  \n", pid, M_sat, rs_sat);
  ...
  sprintf(fname,"final_stream/final_stream_%d.txt",pid);
  fileout = fopen(fname,"w");
  ...
  for(peri_counter=0;peri_counter<N_r_min;peri_counter++)
      ...
      for(inject_counter=0;inject_counter<N_PER_PERI;inject_counter++)
	{
	  ...
	  for(sign=0.;sign<2;sign++)
		...
		{
      // ===============================================================================================================
//    Uncomment to make subhalo massless until just before it does the parameterised flyby
      // if (abs(t-(tmax-0.25*1.0227))>=0.1)
      // {
      //   M_sat=0;
      // }
      // else
